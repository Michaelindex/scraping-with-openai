...
"""
buscador_medicos.v7.py

Versão 7.0 do buscador de médicos com:
- Correção de bugs críticos na extração e salvamento de dados
...
DEBUG_HTML_DIR = os.path.join(DATA_DIR, 'debug_html_v7')
...
def setup_logger(process_id):
    ...
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, f'buscador_medicos_v7_p{process_id}.log'), 'w', 'utf-8')
...
def normalizar_endereco(endereco):
    ...
    endereco = re.sub(r'\br\b', 'rua', endereco)
    endereco = re.sub(r'\bav\b', 'avenida', endereco)
    endereco = re.sub(r'\bapt[o]?\b', 'apartamento', endereco)
    endereco = re.sub(r'\bconj\b', 'conjunto', endereco)
    endereco = re.sub(r'\bs[\/]?n\b', '', endereco)  # Remove "s/n"
    
    # Remove palavras desnecessárias
    endereco = re.sub(r'\bnumero\b', '', endereco)
    endereco = re.sub(r'\bn[o°]\b', '', endereco)
...
def extract_candidates(html, url, logger):
    ...
    try:
        ...
        # Endereço
        candidates['address'] = []
        address_matches = PATTERNS['address'].findall(text)
        for match in address_matches:
            # Limpa e normaliza
            match = match.strip()
            if len(match) > 10 and len(match) < 200:  # Filtra endereços muito curtos ou longos
                candidates['address'].append(match)
        ...
        for match in phone_matches:
            # Limpa e normaliza
            match = match.strip()
            if len(match) >= 13:  # (XX)XXXX-XXXX ou (XX)XXXXX-XXXX
                candidates['phone'].append(match)
        ...
        for match in email_matches:
            ...
            if not any(blacklisted in match for blacklisted in EMAIL_BLACKLIST) and 'subject=' not in match:
                candidates['email'].append(match)
        
        # Complemento
        candidates['complement'] = []
        complement_matches = PATTERNS['complement'].findall(text)
        for match in complement_matches:
            # Limpa e normaliza
            match = match.strip()
            if len(match) > 3 and len(match) < 50:  # Filtra complementos muito curtos ou longos
                candidates['complement'].append(match)
        
        # CEP
        candidates['cep'] = []
        cep_matches = PATTERNS['cep'].findall(text)
        for match in cep_matches:
            # Formata o CEP
            match = formatar_cep(match)
            if match:
                candidates['cep'].append(match)
        
        # Extrai links especiais (tel: e mailto:)
        for a in soup.find_all('a', href=True):
            href = a.get('href', '')
            if href.startswith('tel:'):
                phone = href[4:].strip()
                phone = re.sub(r'\D', '', phone)
                if len(phone) >= 10:  # DDD + número
                    formatted = f"({phone[:2]}){phone[2:6]}-{phone[6:]}"
                    candidates['phone'].append(formatted)
            
            elif href.startswith('mailto:'):
                email = href[7:].strip().lower()
                if '@' in email and not any(blacklisted in email for blacklisted in EMAIL_BLACKLIST):
...
def aggregate_and_rank(candidates, logger):
    ...
    for field, values in candidates.items():
        ...
        # Ordena por frequência e depois por comprimento (mais longo primeiro para endereços)
        if field == 'address':
            ranked = sorted(counter.items(), key=lambda x: (-x[1], -len(x[0])))
        else:
            ranked = sorted(counter.items(), key=lambda x: -x[1])
        
        result[field] = [item[0] for item in ranked]
    
    return result

def validate(field_type, candidates, medico, logger):
    """Valida os candidatos para um campo específico"""
    if not candidates:
        return ""
    ...
    if field_type == 'address':
        for candidate in candidates:
            # Verifica se o endereço tem número
            if re.search(r'\d+', candidate):
                # Verifica se o endereço tem palavras longas (mais de 3 caracteres)
        ...
        for candidate in candidates:
            # Prioriza celulares (DDD + 9)
            if re.search(r'\(\d{2}\)\s?9\d{4}-\d{4}', candidate):
                return candidate
        
        # Se não encontrou celular, retorna o primeiro telefone válido
        for candidate in candidates:
            if re.search(r'\(\d{2}\)\s?\d{4}-\d{4}', candidate):
                return candidate
        ...
        for candidate in candidates:
            ...
            if '@' in candidate and '.' in candidate.split('@')[1]:
                ...
                if not any(blacklisted in candidate.lower() for blacklisted in EMAIL_BLACKLIST):
                    return candidate
        
        return ""
    
    elif field_type == 'complement':
        for candidate in candidates:
            # Verifica se o complemento faz sentido
            if len(candidate) > 3 and not any(texto in candidate.lower() for texto in TEXTOS_REMOVER):
                return candidate
        
        return ""
    
    # Para outros tipos, retorna o primeiro candidato
...
def extrair_numero_endereco(endereco):
    ...
    # Tenta encontrar o padrão "rua, número"
    ...
    # Se não encontrou, retorna o endereço completo e vazio para o número
    return endereco, ""
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Uso: python buscador_medicos.v7.py input.csv output.csv")
        sys.exit(1)
