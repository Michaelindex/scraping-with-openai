...
Versão 3.0 do buscador de médicos com:
...
DEBUG_HTML_DIR = os.path.join(DATA_DIR, 'debug_html_v3')
...
def setup_logger(process_id):
    ...
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, f'buscador_medicos_v3_p{process_id}.log'), 'w', 'utf-8')
...
def obter_cidade_via_cep(cep, logger):
    """Obtém cidade via CEP"""
    try:
        ...
        logger.error(f"Erro ao buscar CEP {cep}: {e}")
    return ''
...
def descobrir_cidade(endereco, cep, uf, driver, logger):
    """Descobre a cidade usando múltiplas estratégias"""
    # Tenta via CEP primeiro
    if cep:
        cidade = obter_cidade_via_cep(cep, logger)
        if cidade:
            return cidade
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Uso: python buscador_medicos.v3.py input.csv output.csv")
        sys.exit(1)
