...
# --- Dados da empresa a ser consultada ---
COMPANY_INFO = {
    "nome_empresa": "AGROFITO" 
}

# --- Prompt para buscar todos os dados da empresa e do contato de TI ---
def create_company_prompt(info):
    return f"""Você é um assistente especializado em encontrar informações detalhadas sobre empresas e seus contatos chave, especificamente da área de Tecnologia da Informação (TI).
Por favor, faça uma busca detalhada e use todas as suas capacidades de pesquisa na web para encontrar os seguintes dados para a empresa e seu contato de TI:

Nome da Empresa: {info['nome_empresa']}

Dados solicitados:
- Nome da Empresa
- CNPJ da Empresa
- Nome do Contato de TI (Ex: Gerente de TI, Coordenador de TI, Head de TI)
- Sobrenome do Contato de TI
- Cargo do Contato de TI (Ex: Gerente de TI, Coordenador de TI)
- Telefone da Empresa (ou do Contato de TI, se disponível)
- Celular do Contato de TI (se disponível)
- E-mail do Contato de TI (profissional)
- Cidade da Empresa
- Estado da Empresa
- CEP da Empresa
...
def run_brutal_comparison_and_save(company_info, num_attempts=3):
    all_results = []
    
    prompt = create_company_prompt(company_info)
    
    print(f"\n--- Iniciando {num_attempts} tentativas para cada API para a empresa: {company_info['nome_empresa']} ---\n")
    ...
    # --- Salvar resultados brutos em output_brutal_company.txt ---
    output_filename = "output_brutal_company.txt"
    with open(output_filename, "w", encoding="utf-8") as f:
        f.write("--- Relatório Detalhado de Testes Brutais de APIs (Empresa) ---\n\n")
        f.write(f"Consulta: Dados da empresa {company_info['nome_empresa']}\n")
...
if __name__ == "__main__":
    ...
    all_test_results = run_brutal_comparison_and_save(COMPANY_INFO, num_attempts=3)
    ...
    try:
        # Valores iniciais que você me informou (usei os valores do teste anterior como base)
        openai_saldo_inicial = 1.93 # Usando o saldo final do teste anterior como inicial para este novo teste
        jina_tokens_inicial = 3952214 # Usando os tokens finais do teste anterior como inicial para este novo teste
        ...
        print("\nGerando relatório de análise em 'analise_comparativa_brutal_company.txt'...")
        
        with open("analise_comparativa_brutal_company.txt", "w", encoding="utf-8") as f_analise:
            f_analise.write("--- RELATÓRIO DE ANÁLISE COMPARATIVA BRUTAL DE APIs (EMPRESA) ---\n\n")
            f_analise.write(f"Data da Análise: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC%z')}\n")
            f_analise.write(f"Consulta: Dados da empresa {COMPANY_INFO['nome_empresa']}\n")
            f_analise.write(f"Número de Tentativas por API: 3\n\n")
            
            f_analise.write("### Saldos e Custos Iniciais (Informados por você antes Deste Teste):\n")
            ...
            f_analise.write("A qualidade é crucial para este teste. Avalie os 'parsed_answer' em `output_brutal_company.txt`:\n")
            f_analise.write("- **Jina DeepSearch:** Verifique se as tentativas que não deram timeout contêm dados válidos e completos no 'raw_answer' e o quão bem o 'parsed_answer' conseguiu extrair o JSON. Espera-se que a busca por empresas seja mais direta.\n")
            f_analise.write("- **OpenAI ChatGPT (`gpt-4o-mini-search-preview-2025-03-11`):** Avalie se os campos do JSON estão preenchidos corretamente e com precisão para AGROFITO e seu contato de TI. Espera-se que ele seja consistente na entrega do JSON.\n")
            f_analise.write("  *Critério de Sucesso de Qualidade:* JSON válido e campos preenchidos com dados corretos e relevantes para AGROFITO e seu contato de TI.\n\n")
            
            f_analise.write("#### Tempo de Resposta:\n")
            f_analise.write("- **Jina DeepSearch:** Monitore se ele ainda está dando timeouts ou se o tempo de resposta melhorou para esta nova consulta. Um tempo de resposta consistente é fundamental.\n")
            f_analise.write("- **OpenAI ChatGPT:** O tempo médio deve ser significativamente menor e mais consistente.\n\n")

            f_analise.write("#### Custo:\n")
            f_analise.write("- Compare os 'Custo Total Observado' de cada API. O custo do OpenAI será o mais preciso.\n")
            f_analise.write("- Para o Jina, se ele falhar novamente, mesmo que o custo por token seja baixo, o custo-benefício é ruim.\n")
            
            f_analise.write("### Conclusão e Recomendação Final com Base Neste Teste Brutal (Empresa):\n")
            f_analise.write("Com base nos resultados consolidados deste 'teste brutal' para a empresa AGROFITO:\n")
            f_analise.write("- Se o **Jina DeepSearch** continuar a exibir problemas de timeout ou retornos não-JSON para esta consulta de empresa, a conclusão do teste anterior para dados de médicos se mantém: instabilidade e dificuldade de parseabilidade. No entanto, se ele entregar resultados, reavalie sua performance.\n")
            f_analise.write("- O **OpenAI ChatGPT (`gpt-4o-mini-search-preview-2025-03-11`)** deve continuar a demonstrar sua **consistência e capacidade de resposta**. A qualidade dos dados extraídos para a empresa e o contato de TI será o fator chave.\n\n")
            f_analise.write("A decisão final dependerá da **qualidade dos dados retornados para a AGROFITO e seu contato de TI**, além da **consistência na entrega do JSON e dos tempos de resposta**.\n")
            
        print("Relatório de análise completo salvo em 'analise_comparativa_brutal_company.txt'.")
