...
"""
buscador_medicos.v9.py

Versão 9.0 do buscador de médicos com:
- Reforço na extração de CEPs com validação cruzada e múltiplos fallbacks
- Limpeza de texto aprimorada para evitar excesso de informações irrelevantes
- Treinamento mais específico da IA para cada tipo de campo
- Manutenção da lógica de extração de endereços da v8 (restaurada da v6)
- Sistema de cache expandido para maior eficiência
- Processamento paralelo otimizado
- Validação rigorosa de resultados antes do salvamento
...
DEBUG_HTML_DIR = os.path.join(DATA_DIR, 'debug_html_v9') # Atualizado para v9
...
def setup_logger(process_id):
    ...
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, f'buscador_medicos_v9_p{process_id}.log'), 'w', 'utf-8') # Atualizado para v9
...
# Carrega CEPs manuais (novo na v9)
...
# Função mantida da v8 com melhorias para v9
...
# Funções de busca de CEP (mantidas da v8 com melhorias para v9)
...
def buscar_cep_com_cascata(rua, cidade, uf, driver, logger, medico=None):
    ...
    # 0. Verifica CEPs manuais (novo na v9)
    if medico:
...
# Função para limpar texto extenso (nova na v9)
def limpar_texto_extenso(texto, tipo_campo, logger):
    ...
    if tipo_campo == 'address':
        ...
        # Limita o complemento a 50 caracteres
        texto = texto[:50]
        ...
        if all_candidates_raw:
            ...
            if ranked.get('address'):
                ...
                if validar_endereco(endereco_completo):
                    ...
                    # Limpa texto extenso (novo na v9)
            ...
            if result.get('Address A1') and result.get('City A1'):
                ...
                cep_data = buscar_cep_com_cascata(
                    ...
                    driver,
                    logger,
                    m  # Passa o médico para verificar CEPs manuais (novo na v9)
                )
            ...
            # Validação final dos dados (novo na v9)
            for campo in ['Endereco Completo A1', 'Address A1', 'Numero A1', 'Complement A1', 
                          'Bairro A1', 'City A1', 'State A1', 'Phone A1', 'E-mail A1']:
                if campo in result and result[campo]:
                    ...
                    if len(str(result[campo])) > 100 or any(exp in str(result[campo]).lower() for exp in ["aqui está", "encontrei", "segue"]):
                        ...
                        result[campo] = limpar_texto_extenso(str(result[campo]), campo.lower(), logger)
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Uso: python buscador_medicos.v9.py input.csv output.csv")
        sys.exit(1)
