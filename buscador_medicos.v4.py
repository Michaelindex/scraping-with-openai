...
Versão 4.0 do buscador de médicos com:
...
- Busca de CEP via web ao final do processo
...
- Processamento paralelo para maior velocidade
- Sistema de treinamento da IA para melhor precisão
- Busca de CEP baseada em rua e cidade já coletadas
"""
import sys
import csv
import re
...
DEBUG_HTML_DIR = os.path.join(DATA_DIR, 'debug_html_v4')
...
def setup_logger(process_id):
    ...
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, f'buscador_medicos_v4_p{process_id}.log'), 'w', 'utf-8')
...
def buscar_cep_por_endereco(rua, cidade, driver, logger):
    ...
    if ceps:
        logger.info(f"CEP encontrado: {ceps[0]}")
        return ceps[0]
    ...
    if ceps:
        logger.info(f"CEP encontrado na segunda tentativa: {ceps[0]}")
        return ceps[0]
...
def process_medico(m, driver, logger):
    ...
    try:
        ...
        if all_candidates:
            ...
            # Busca CEP ao final do processo, usando rua e cidade já coletados
            if result.get('Address A1') and result.get('City A1'):
                logger.info("Buscando CEP com base na rua e cidade")
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Uso: python buscador_medicos.v4.py input.csv output.csv")
        sys.exit(1)
