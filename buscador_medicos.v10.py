...
"""
buscador_medicos.v10.py

Versão 10.0 do buscador de médicos com:
- Integração de CEPs manuais para casos específicos
- Prompts refinados para IA com exemplos específicos por campo
- Limpeza de texto aprimorada para todos os campos
- Validação final rigorosa antes do salvamento
- Manutenção da lógica de extração de endereços da v9
- Sistema de cache expandido para maior eficiência

Aprimoramentos:
- Arquivo de CEPs manuais para casos conhecidos
...
DEBUG_HTML_DIR = os.path.join(DATA_DIR, 'debug_html_v10') # Atualizado para v10
...
def setup_logger(process_id):
    ...
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, f'buscador_medicos_v10_p{process_id}.log'), 'w', 'utf-8') # Atualizado para v10
...
def gerar_chave_medico(medico):
    """Gera uma chave única para o médico (para CEPs manuais)"""
    firstname = medico.get('Firstname', '').strip().upper()
    lastname = medico.get('LastName', '').strip().upper()
    uf = medico.get('UF', '').strip().upper()
...
# Exemplos específicos para cada tipo de campo (novo na v10)
EXEMPLOS_CAMPOS = {
    'address': [
...
# Função para gerar prompt específico por campo (novo na v10)
def gerar_prompt_especifico(tipo_campo, texto_html, logger):
    """Gera um prompt específico para o tipo de campo"""
    exemplos = EXEMPLOS_CAMPOS.get(tipo_campo, [])
    exemplos_str = "\n".join([f"- {ex}" for ex in exemplos])
    
    # Instruções específicas por tipo de campo
    instrucoes = {
        'address': "Extraia APENAS o nome da rua/avenida (sem número, complemento, bairro ou cidade)",
        'phone': "Extraia APENAS o número de telefone no formato (XX) XXXX-XXXX ou (XX) XXXXX-XXXX",
        'email': "Extraia APENAS o endereço de e-mail completo, sem texto adicional",
        'complement': "Extraia APENAS o complemento do endereço (sala, conjunto, bloco, etc.)",
        'city': "Extraia APENAS o nome da cidade, sem estado ou país",
        'state': "Extraia APENAS a sigla do estado (2 letras)",
        'bairro': "Extraia APENAS o nome do bairro, sem cidade ou estado"
    }
    
    instrucao = instrucoes.get(tipo_campo, "Extraia a informação solicitada")
...
REGRAS IMPORTANTES:
1. Retorne APENAS a informação solicitada, sem explicações ou texto adicional
2. Não inclua frases como "Aqui está" ou "Encontrei"
3. Não inclua marcadores de lista ou numeração
4. Não inclua aspas ou parênteses
5. Se não encontrar a informação, retorne uma string vazia
6. Não invente informações
7. Não inclua textos como "Endereço:", "Telefone:", etc.
8. Retorne apenas UM resultado, o mais relevante
9. Não inclua URLs ou links
10. Formate exatamente como nos exemplos abaixo
...
"""
    ...
    logger.info(f"Prompt gerado para campo {tipo_campo}")
    return prompt
    ...
    try:
        ...
        if all_candidates_raw:
            ...
            if ranked.get('address'):
                ...
                endereco_completo = ranked['address'][0]
                if validar_endereco(endereco_completo):
                    logger.info(f"Endereço validado: {endereco_completo}")
                    # Limpa texto extenso (aprimorado na v10)
                    endereco_completo = limpar_texto_extenso(endereco_completo, 'address', logger)
            ...
            if ranked.get('phone'):
                ...
                for phone in ranked['phone'][:3]:
                    if validar_telefone(phone):
                        # Limpa texto extenso (aprimorado na v10)
                        phone = limpar_texto_extenso(phone, 'phone', logger)
            ...
            if ranked.get('email'):
                ...
                for email in ranked['email'][:3]:
                    if validar_email(email):
                        # Limpa texto extenso (aprimorado na v10)
                        email = limpar_texto_extenso(email, 'email', logger)
            ...
            if ranked.get('complement'):
                ...
                # Limpa texto extenso (aprimorado na v10)
                complement = limpar_texto_extenso(ranked['complement'][0], 'complement', logger)
                result['Complement A1'] = complement
            ...
            if result.get('Address A1') and not result.get('City A1'):
                ...
                # Limpa texto extenso (aprimorado na v10)
                cidade = limpar_texto_extenso(cidade, 'city', logger)
                result['City A1'] = cidade
            ...
            if result.get('Address A1') and result.get('City A1'):
                ...
                cep_data = buscar_cep_com_cascata(
                    ...
                    driver,
                    logger,
                    m  # Passa o médico para verificar CEPs manuais
                )
                ...
                if cep_data:
                    ...
                    if 'bairro' in cep_data and cep_data['bairro'] and not result.get('Bairro A1'):
                        # Limpa texto extenso (aprimorado na v10)
                        bairro = limpar_texto_extenso(cep_data['bairro'], 'bairro', logger)
                        result['Bairro A1'] = bairro
                    
                    if 'complemento' in cep_data and cep_data['complemento'] and not result.get('Complement A1'):
                        # Limpa texto extenso (aprimorado na v10)
                        complemento = limpar_texto_extenso(cep_data['complemento'], 'complement', logger)
                        result['Complement A1'] = complemento
                    
                    if 'localidade' in cep_data and cep_data['localidade']:
                        # Limpa texto extenso (aprimorado na v10)
                        localidade = limpar_texto_extenso(cep_data['localidade'], 'city', logger)
                        result['City A1'] = localidade
                    
                    if 'uf' in cep_data and cep_data['uf']:
                        # Limpa texto extenso (aprimorado na v10)
                        uf = limpar_texto_extenso(cep_data['uf'], 'state', logger)
                        result['State A1'] = uf
            
            # Validação final dos dados (aprimorada na v10)
            for campo, tipo in [
                ('Endereco Completo A1', 'address'), 
                ('Address A1', 'address'), 
                ('Numero A1', 'numero'), 
                ...
                if campo in result and result[campo]:
                    # Verifica se o campo tem texto explicativo ou é muito longo
                    if len(str(result[campo])) > 100 or any(exp in str(result[campo]).lower() for exp in ["aqui está", "encontrei", "segue"]):
                        logger.warning(f"Campo {campo} contém texto explicativo ou é muito longo: {result[campo]}")
                        # Limpa novamente para garantir
                        result[campo] = limpar_texto_extenso(str(result[campo]), tipo, logger)
    ...
    try:
        ...
        for i, medico in enumerate(medicos_batch):
            try:
                ...
                # Reinicia o driver a cada 5 médicos para evitar vazamento de memória
                ...
                # Salva resultados a cada 3 médicos para evitar perda de dados
                if len(results) >= 3:
...
def run_parallel(inp, outp, num_processes=None):
    ...
    batch_size = max(1, min(3, math.ceil(len(medicos) / num_processes)))
